export default {
  setMusicList(state, musicList) { // 设置音乐列表
  ...
  setDrawerMusicList(state, drawerMusicList) {  // 抽屉音乐列表
  ...
  setHistoryList(state, payload) {
    if (payload.type === 'unshift') {
      ...
      if (
        state.historyList.findIndex((item) => item.id == payload.data.id) !== -1
      )
        return
  ...
  setPersonalFm(state, payload) {
    if (payload.type === 'set') {
      state.musicList = payload.data // 设置音乐列表
      state.currenIndex = 0
